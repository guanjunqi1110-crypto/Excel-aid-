...
_EMPTY_RECS: pd.DataFrame = pd.DataFrame(
    columns=["Title", "Module", "Level", "Type", "Description", "_module_id_raw"]
)
...
def _finalize_material_frame(
    ...
    return t[["Title", "Module", "Level", "Type", "Description", "_module_id_raw"]]
...
def get_recommended_materials(
    ...
    out = _finalize_material_frame(
        picked, title_col, module_name_col, level_col, type_col, desc_col, module_id_col
    )
    return out, rec_mode
...
In **Recommended learning path** and **Recommended resources**, map actions explicitly to the **titles** of the approved resources when available.
"""
...
def generate_ai_study_guide(prompt: str) -> str:
    ...
    coach_system = (
        ...
        "content-pack / Material Library items the user listed. Be concise; avoid generic filler."
    )
    try:
...
def main() -> None:
    ...
    try:
        ...
        st.error(
            "Workbook not found at `./Excel_Readiness_AI_Coach_Content_Pack.xlsx`.\n\n"
